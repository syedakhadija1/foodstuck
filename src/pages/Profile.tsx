...
export const ProfilePage = () => {
  ...
  const imgRef = useRef<HTMLInputElement | null>(null); // Updated type
  ...
  useEffect(() => {
    if (!user) {
      navigateFromUser("/sign-up");
    }
  }, [user, navigateFromUser]); // Fixed missing dependency
  ...
  }, [img, user, update]); // Fixed missing dependencies
  ...
  const editNameHandler = async () => {
    if (user) {
      ...
      setIsActiveNameInput(false); // Fixed state toggle logic
    }
  };
